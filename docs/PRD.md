...
#### 3.2.3 Dossiers de réservation (modifié)

**Changement :** Les réservations sont désormais chargées depuis la DB backend (table `local_bookings`) au lieu d'être fetchees directement depuis OpenPro.

**Endpoints :**
- `GET /api/suppliers/:idFournisseur/local-bookings` - Liste toutes les réservations depuis la DB (toutes plateformes confondues)
  - **Changement :** Retourne toutes les réservations stockées en DB (Directes, OpenPro, plateformes externes)
  - Filtres optionnels : `debut`, `fin`, `idHebergement`
- `GET /api/suppliers/:idFournisseur/local-bookings/:id` - Détail d'une réservation
...
- `POST /api/suppliers/:idFournisseur/rate-types` - Créer un type de tarif
  - **Changement :** 
    1. Sauvegarde d'abord en DB (approche DB-first)
    2. Export ensuite vers OpenPro automatiquement
    3. Met à jour l'ID OpenPro dans la DB après création
...
- `DELETE /api/suppliers/:idFournisseur/rate-types/:idTypeTarif` - Supprimer un type de tarif
  - **Changement :** 
    1. Supprime d'abord toutes les données tarifaires associées en DB
    2. Supprime ensuite dans OpenPro
    3. Supprime enfin le plan tarifaire en DB
...
- Affichage des avertissements de synchronisation via `GET /api/startup-warnings` (optionnel)
